...
# if person exits in canteen
def outsideCanteen(name):
    with open('insideCanteen.csv', 'r+') as f:
        ...
        if name in nameList:
            data = pd.read_csv("insideCanteen.csv", index_col ="Name" )
            data.drop([name,], inplace = True)
            data.to_csv("insideCanteen.csv", index=True)
            print(f"Hi {name}, your exit has been recorded . \n \n")
        else:
            print(f"\n \nHi {name}, You are already out. Enjoy your day :)")
        canteenCount()
